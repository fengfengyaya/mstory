...
export default {
  ...
  methods: {
    ...
    loadMore () {
      ...
      axios
        .get(
          `/api/recommend/storyList?per_page=10&page=${
            this.current
          }&user_id=48a192eac3b1f9d6798558fda3a29acc&session_id=48a192eac3b1f9d6798558fda3a29acc&action=1`
        )
        .then(res => {
  ...
  mounted () {
    axios
      .get(
        '/api/story/list?order=desc&per_page=10&from_id=2&page=1&sortby=add_time'
      )
      .then(res => {
        console.log(res.data)
        this.datalist = res.data.data.story_data
      })
  }
}
</script>
